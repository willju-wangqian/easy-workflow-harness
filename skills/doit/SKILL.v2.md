...
# doit — Binary-Backed Dispatcher (v2 Draft)

> **Phase 1 draft.** This shim replaces `skills/doit/SKILL.md` in Phase 2
> of the rewrite described in `specs/dispatcher-binary-v2.md`. Until
> then, the production dispatcher remains the v1 prose SKILL.md; this
> file is exercised only via direct CLI smoke tests.
...
**First instruction** is pre-baked via shell injection so there is no wasted
turn. The very first line of this skill is:

    !`ewh start "$ARGUMENTS"`

Read the `ACTION:` block in that output and then follow the loop below.
