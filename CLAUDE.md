...
/ewh:doit design "description"         # design a rule, agent, or workflow (replaces create)
...
**Subcommands** (`init`, `cleanup`, `design`, `expand-tools`, `list`) are implemented in `src/commands/*.ts`. No agents, rules, compliance, or artifact workspace — each subcommand is a multi-turn state machine (stored in `RunState.subcommand_state`) that emits one action per turn.
...
Prefer `/ewh:doit design` over manual file creation — it interviews, proposes, and gates before writing.
