...
The dispatcher (`skills/doit/SKILL.md`) is the core. When a user runs `/ewh:doit <name>`, it first resolves the name:
...
2. Prepares `.ewh-artifacts/` workspace (warns if stale artifacts exist and waits for user confirmation before clearing)
3. For each step: evaluates preconditions (`requires:`), checks gate, resolves scripts (§1d — runs cached/proposed scripts for mechanical steps, saving tokens), validates early (already-done/trivial), resolves rules, builds prompt (with `reads:` and `artifact:` directives), validates context, spawns agent (with tool lists from overrides generated by `expand-tools`), checks `AGENT_COMPLETE` sentinel, verifies artifact written
4. If sentinel is absent (partial output): spawns one continuation agent to finish remaining work; if that also fails, splits into 30-item parallel chunks and merges results
5. After steps with `severity: critical` rules: spawns the compliance agent to verify
6. On completion: cleans up `.ewh-artifacts/`
...
**Harness Config**: The `init` subcommand appends a `## Harness Config` section to the project's CLAUDE.md. The dispatcher reads this for test commands, source patterns, etc. Agents receive it under `## Project Context`.
