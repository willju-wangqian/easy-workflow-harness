...
var ARTIFACTS_DIR = ".ewh-artifacts";
...
function activeMarker(projectRoot, runId) {
...
async function atomicWrite(path2, body) {
  await fs.mkdir(dirname(path2), { recursive: true });
  const tmp = `${path2}.tmp-${randomBytes(4).toString("hex")}`;
...
async function markActive(projectRoot, runId) {
...
var SCRIPTS_DIR = join2(".claude", "ewh-scripts");
function scriptCachePath(projectRoot, workflow, stepName) {
  return resolve2(projectRoot, SCRIPTS_DIR, workflow, `${stepName}.sh`);
}
function hashCachePath(projectRoot, workflow, stepName) {
  return resolve2(projectRoot, SCRIPTS_DIR, workflow, `${stepName}.hash`);
}
async function readCachedScript(projectRoot, workflow, stepName) {
  const path2 = scriptCachePath(projectRoot, workflow, stepName);
  try {
  ...
  let storedHash = null;
  try {
    storedHash = (await fs2.readFile(hashCachePath(projectRoot, workflow, stepName), "utf8")).trim();
  } catch {
  }
  return { path: path2, storedHash };
}
async function writeCachedScript(projectRoot, workflow, stepName, script, hash) {
  const path2 = scriptCachePath(projectRoot, workflow, stepName);
  await fs2.mkdir(dirname2(path2), { recursive: true });
  await fs2.writeFile(path2, script, { mode: 493 });
  await fs2.writeFile(hashCachePath(projectRoot, workflow, stepName), hash + "\n", "utf8");
}
async function listCachedScripts(projectRoot, workflow) {
  const dir = resolve2(projectRoot, SCRIPTS_DIR, workflow);
  try {
    const entries = await fs2.readdir(dir);
    return entries.filter((e) => e.endsWith(".sh")).map((e) => ({ stepName: e.slice(0, -3), path: join2(dir, e) }));
  } catch {
    return [];
  }
}
...
// src/state/sentinel.ts
var SENTINEL = "AGENT_COMPLETE";
var DEFAULT_SCAN_LINES = 10;
function checkSentinel(content, scanLines = DEFAULT_SCAN_LINES) {
...
function extractSection(content, heading) {
  ...
  const section = rel === -1 ? content.slice(start) : content.slice(start, start + rel);
  return section.trim() || void 0;
}
...
${bodies.join("\n\n---\n\n")}`);
  ...
  if (priorSteps.length > 0) {
    const formatted = priorSteps.map(({ ref, summary }) => formatPriorStep(ref, summary)).join("\n\n");
...
function extractFilesModified(content) {
  const lines = content.split("\n");
  const startIdx = lines.findIndex(
    (l) => /^-?\s*files_modified\s*:/i.test(l)
  );
  if (startIdx === -1) return void 0;
  const inline = lines[startIdx]?.match(/:\s*\[(.+)\]/);
  if (inline?.[1]) {
    const items2 = inline[1].split(",").map((s) => s.trim().replace(/^['"]|['"]$/g, "")).filter(Boolean);
...
// src/scripts/evaluate.ts
function isScriptable(step) {
  return !!(step.agent && !step.chunked && !step.reads?.length && !step.artifact && !step.context?.length);
}
async function evaluateScript(projectRoot, workflow, step) {
  if (step.script) {
    const scriptPath = step.script.startsWith("/") ? step.script : resolve4(projectRoot, step.script);
...
async function writeEwhStateFile(projectRoot, state) {
...
// src/chunking/plan.ts
var DEFAULT_CHUNK_SIZE = 8;
async function enumerateFiles(patterns, projectRoot) {
...
function splitIntoChunks(files, size = DEFAULT_CHUNK_SIZE) {
...
function parsePatternsContent(content) {
  ...
  if (Array.isArray(parsed)) {
    const include2 = parsed.filter((x) => typeof x === "string");
  ...
  const obj = parsed;
  const include = Array.isArray(obj.include) ? obj.include.filter((x) => typeof x === "string") : [];
  if (include.length === 0) {
    throw new Error("patterns.include must be a non-empty array of glob strings");
  }
  const exclude = Array.isArray(obj.exclude) ? obj.exclude.filter((x) => typeof x === "string") : void 0;
...
async function mergeChunkArtifacts(params) {
  ...
  const absTarget = resolve5(projectRoot, targetArtifact);
  const parts = [];
  let present = 0;
  let missing = 0;
...
// src/continuation/detect.ts
var LIST_ITEM_RE = /^\s*(?:\d+[.)]\s+|[-*•]\s+)(.+)$/;
function detectListItems(partial) {
  const items = [];
...
// src/continuation/split.ts
var DEFAULT_SPLIT_SIZE = 30;
function splitItems(items, size = DEFAULT_SPLIT_SIZE) {
...
async function transitionStep(step, event, run, opts) {
...
async function enterPending(step, _event, run, opts) {
...
async function enterScriptEvalInline(step, run, opts) {
  if (step.agent || step.script) {
    const decision = await evaluateScript(opts.projectRoot, run.workflow, step);
    switch (decision.kind) {
      case "explicit":
      case "cached": {
        const staleNote = decision.kind === "cached" && decision.stale ? `\u26A0 Cached script may be stale (step definition has changed since last approval).` : "";
        const scriptRunState = {
          phase: "script_run",
...
function generateScriptTemplate(step, workflow) {
  const lines = [
    "#!/usr/bin/env bash",
...
async function handleScriptProposeReport(step, state, event, run, opts) {
  if (event.kind !== "report" || event.report.kind !== "decision") {
...
async function executeScript(step, state, staleNote, run, opts) {
  try {
...
function scriptFailGate(step, state, errorOutput, staleNote, run) {
  const runId = run.run_id;
  const stepIdx = run.current_step_index;
  const lines = [
  ...
  lines.push(
    ...
    step.agent ? `  use agent instead:  ewh report --run ${runId} --step ${stepIdx} --decision no` : `  skip this step:     ewh report --run ${runId} --step ${stepIdx} --decision no`,
...
async function handleScriptRunReport(step, state, event, run, opts) {
  if (event.kind !== "report" || event.report.kind !== "decision") {
    throw new Error(
...
async function enterAgentRun(step, run, opts) {
...
async function handleAgentRunReport(step, state, event, run, opts) {
  ...
  if (event.report.kind === "error") {
    ...
    const nextState = { ...state, retries: newRetries };
  ...
  if (event.report.kind === "decision") {
    ...
    const resetState = { ...state, retries: 0 };
    return {
      next: resetState,
      instruction: {
        kind: "tool-call",
...
async function runComplianceCheck(step, rules, summary, run, opts) {
...
async function handleComplianceReport(step, state, event, run, opts) {
  if (event.kind !== "report" || event.report.kind !== "decision") {
...
async function runVerifyCommands(rules, cwd) {
  const failures = [];
...
function complianceFailInstruction(step, failures, run) {
  const runId = run.run_id;
  const stepIdx = run.current_step_index;
...
function buildAgentCallBody(step, state, run, meta) {
  const lines = [
    `Tool: Agent`,
    `Args:`,
...
function collectPriorSteps(refs, run) {
...
async function enterChunkPlan(step, run, opts) {
  if (!step.agent) {
    throw new Error(
...
async function handleChunkPlanEvent(step, event, run, opts) {
  if (event.kind === "enter") {
...
async function beginChunkRunning(step, patterns, run, opts) {
  ...
  const retries = new Array(chunks.length).fill(0);
  const incremental = !!agent.incremental;
  if (incremental) {
    const header = step.artifact ? `# ${step.artifact} \u2014 chunk findings` : `# ${step.name} \u2014 chunk findings`;
    await Promise.all(
...
async function handleChunkRunningEvent(step, state, event, run, opts) {
  if (event.kind === "enter") {
    ...
    return {
      ...
      instruction: {
        ...
        body: [
          ...
          `Error: ${event.report.message}`,
          state.incremental ? `Incremental agent: retry resumes from the on-disk artifact.` : `Non-incremental: retry re-runs the chunk from scratch.`,
          ``,
          `Options:`,
          `  retry:  ewh report --run ${runId} --step ${stepIdx} --decision yes`,
...
async function advanceChunkOrMerge(step, state, run, opts) {
  const idx = state.chunk_index;
...
async function enterChunkMerge(step, state, run, opts) {
  let mergeNote;
  if (step.artifact) {
...
async function loadAgentAndRules(step, run, opts) {
...
async function buildChunkInstruction(step, state, idx, run, opts, extra) {
  const runId = run.run_id;
  const stepIdx = run.current_step_index;
...
function buildChunkDescription(step, idx, total, files, artifactPath, incremental) {
  const lines = [
...
function buildContinuationAgentCall(step, promptPath, resultPath, agent, run) {
  const tools = agent.tools ? agent.tools.join(", ") : "default";
  const runId = run.run_id;
  const stepIdx = run.current_step_index;
  return {
...
async function handleContinuationEvent(step, state, event, run, opts) {
  if (event.kind === "enter") {
    const agent = await loadAgent(step.agent, opts.pluginRoot, opts.projectRoot);
    return {
      next: state,
...
async function enterSplit(step, contState, run, opts) {
  ...
  const itemGroups = items.length > 0 ? splitItems(items, DEFAULT_SPLIT_SIZE) : [[]];
  ...
  const next = {
    phase: "split",
    chunks,
    completed: new Array(chunks.length).fill(false),
...
async function buildSplitInstruction(step, state, idx, run, opts) {
...
async function handleSplitEvent(step, state, event, run, opts) {
  if (event.kind === "enter") {
...
async function advanceSplitOrMerge(step, state, run, opts) {
  const nextCompleted = [...state.completed];
...
async function executeSplitMerge(step, state, run, opts) {
  const parts = [];
  let present = 0;
  let missing = 0;
...
async function enterArtifactVerify(step, rules, summary, run, opts) {
  if (!step.artifact) {
...
async function handleArtifactVerifyEvent(step, state, event, run, opts) {
  if (event.kind === "enter") {
...
function completeNoop(step, run) {
  const next = {
    phase: "complete",
    summary: {
      step_name: step.name,
...
function doneOrNext(run, justFinished, outcome) {
...
`;
async function buildListInstruction(opts) {
  const body = await buildListBody(opts);
  return { kind: "done", body };
}
async function buildListBody(opts) {
...
async function listMdBasenames(dir, recursive) {
  ...
  const names = matches.map((m) => m.split(/[\\/]/).pop() ?? m).filter((m) => m.endsWith(".md")).map((m) => m.slice(0, -3));
...
async function startBareCleanup(opts) {
...
async function startManageTasks(opts) {
  const subState = { kind: "cleanup-manage", phase: "scan" };
  const instruction = {
    kind: "bash",
    body: [
...
async function continueCleanup(run, report, opts) {
...
function continueBareCleanup(run, sub, report) {
  if (sub.phase === "task-failed") {
    ...
    if (report.decision === "no") {
      ...
      run.subcommand_state = next3;
      return advanceOrSummarize(run, next3);
    }
    const next2 = {
      kind: "cleanup",
      phase: "running",
    ...
    run.subcommand_state = next2;
    return advanceOrSummarize(run, next2);
  }
...
function advanceOrSummarize(run, sub) {
...
function buildTaskInstruction(task, index, total) {
...
async function continueManageTasks(run, sub, report, opts) {
  if (sub.phase === "scan") {
    if (report.kind === "error") {
...
async function persistCleanupTasks(projectRoot, tasks) {
...
async function continueInit(run, report, opts) {
  const sub = run.subcommand_state;
...
async function readScanFile(path2) {
  ...
  const r = parsed;
  const pick = (k) => typeof r[k] === "string" && r[k].length > 0 ? r[k] : void 0;
  return {
...
function buildHarnessConfigSection(scan) {
  const v = (x) => x ?? "none";
  return [
    "## Harness Config",
    "",
...
async function collectEntries(baseDir, type, scope, relativeFrom) {
...
async function buildCatalog(projectRoot, pluginRoot) {
  const [
    pluginWorkflows,
    pluginAgents,
    pluginRules,
...
function parseProposedInput(raw) {
  ...
  const slices = steps.map((raw2, i) => validateSlice(raw2, i));
  let parsedOrder;
  if (order !== void 0) {
...
function validateContextArray(v, where) {
  if (!Array.isArray(v)) {
    throw new Error(`${where}: 'context' must be an array`);
  }
  return v.map((raw, i) => {
...
function validateRequiresArray(v, where) {
  if (!Array.isArray(v)) {
    throw new Error(`${where}: 'requires' must be an array`);
  }
  return v.map((raw, i) => {
...
function diffContract(current, proposed) {
  const errors = [];
  const updated = [];
  const added = [];
  const deleted = [];
  ...
  const pathRewrites = /* @__PURE__ */ new Map();
  const nameRewrites = /* @__PURE__ */ new Map();
  const deletes = proposed.steps.filter((s) => s._delete === true);
  const renames = proposed.steps.filter((s) => s._rename_from !== void 0);
  const rest = proposed.steps.filter(
    (s) => s._delete !== true && s._rename_from === void 0
  );
  ...
  let finalOrder;
  let reordered = false;
  if (proposed._order !== void 0) {
    const requested = proposed._order;
    const requestedSet = new Set(requested);
    const mergedSet = new Set(byName.keys());
    if (requested.length !== mergedSet.size || [...mergedSet].some((n) => !requestedSet.has(n)) || requested.some((n) => !mergedSet.has(n))) {
      errors.push(
...
function inferOrder(startingOrder, byName, nameRewrites) {
  const out = [];
  const seen = /* @__PURE__ */ new Set();
...
function cloneStep(s) {
  return {
    ...
    requires: s.requires.map((r) => ({ ...r })),
    chunked: s.chunked,
    script: s.script,
    script_fallback: s.script_fallback
  };
}
function applyFields(base, slice) {
  return {
    name: base.name,
    // caller sets `name` explicitly for renames
    ...
    context: slice.context ? slice.context.map((c) => ({ ...c })) : base.context.map((c) => ({ ...c })),
    requires: slice.requires ? slice.requires.map((r) => ({ ...r })) : base.requires.map((r) => ({ ...r })),
    chunked: slice.chunked ?? base.chunked,
    script: slice.script === void 0 ? base.script : slice.script,
    script_fallback: slice.script_fallback ?? base.script_fallback
  };
}
function sliceToStep(slice) {
  return {
    name: slice.name,
    agent: slice.agent ?? "",
    ...
    requires: slice.requires ? slice.requires.map((r) => ({ ...r })) : [],
    chunked: slice.chunked ?? false,
    script: slice.script === void 0 ? null : slice.script,
    script_fallback: slice.script_fallback ?? "gate"
  };
}
function applyRewrites(step, nameRewrites, pathRewrites) {
  const context = step.context.map((c) => {
    if (c.type === "artifact" && pathRewrites.has(c.ref)) {
      return { type: "artifact", ref: pathRewrites.get(c.ref) };
    }
    return { ...c };
  });
...
async function checkIntegrity(merged, opts) {
  const issues = [];
...
function renderDiffSummary(diff, integrity) {
  const lines = [];
  ...
  if (!diff.added.length && !diff.updated.length && !diff.renamed.length && !diff.deleted.length && !diff.reordered) {
...
async function startDesign(opts) {
  ...
  await fs18.mkdir(paths.proposedDir, { recursive: true });
  const catalog = await buildCatalog(opts.projectRoot, opts.pluginRoot);
  await fs18.writeFile(paths.catalog, JSON.stringify(catalog, null, 2), "utf8");
  await fs18.writeFile(paths.description, description + "\n", "utf8");
...
async function continueInterview(run, sub, report, opts) {
  ...
  return renderShapeGate(run, parsed.proposal, paths, notes.length > 0 ? notes : void 0);
}
async function bounceToInterview(run, sub, paths, errors) {
  const note = [
    "",
  ...
  await fs18.appendFile(paths.description, note, "utf8");
...
async function continueShapeGate(run, sub, report, opts) {
  ...
  if (report.kind === "result") {
    ...
    const instruction = (await fs18.readFile(report.result_path, "utf8")).trim();
    const paths = designPaths(opts.projectRoot, run.run_id);
    const note = [
      "",
...
function renderShapeGate(run, proposal, paths, notes) {
...
async function readShape(path2) {
  let raw;
  try {
    raw = await fs18.readFile(path2, "utf8");
  } catch (e) {
  ...
  try {
    ...
    return { ok: false, error: `shape.json is not valid JSON: ${e.message}` };
  }
  return validateShapeShape(parsed);
}
function validateShapeShape(input) {
...
function validateShape(proposal, catalog) {
  const errors = [];
...
async function readCatalog(path2) {
  const raw = await fs18.readFile(path2, "utf8");
...
async function applyScopeRewrites(proposalPath, proposal) {
  let count = 0;
  ...
  if (count > 0) {
    await fs18.writeFile(proposalPath, JSON.stringify(proposal, null, 2), "utf8");
  }
  return count;
}
async function atomicCopy(src, dst) {
  await fs18.mkdir(dirname6(dst), { recursive: true });
  const body = await fs18.readFile(src, "utf8");
  const tmp = `${dst}.tmp-${randomBytes3(4).toString("hex")}`;
  const fh = await fs18.open(tmp, "w");
  try {
    await fh.writeFile(body, "utf8");
    await fh.sync();
  } finally {
...
async function continueWrite(run, sub, opts) {
  ...
  const proposal = parsed.proposal;
  const paths = designPaths(opts.projectRoot, run.run_id);
  const CLASS_ORDER = { rule: 0, agent: 1, workflow: 2 };
...
function buildAuthorInstruction(args) {
  ...
  const existingLine = artifact.op === "update" ? [`    existing_path: ${existingPathForArtifact(artifact, opts)}`] : [];
  const body = [
    "Tool: Agent",
    "Args:",
...
async function renderFileGate(run, proposal, fileIndex, paths) {
  ...
  lines.push("");
  if (artifact.op === "create") {
    const body = await fs18.readFile(stagedPath, "utf8");
    lines.push("--- staged file body ---");
    lines.push(body.trimEnd());
  } else {
    const diffPath = stagedPath + ".diff";
    try {
      const diff = await fs18.readFile(diffPath, "utf8");
...
async function continueAuthor(run, sub, report, opts) {
  if (report.kind === "error") {
...
async function continueFileGate(run, sub, report, opts) {
  const parsed = await readShape(sub.proposal_path);
  if (!parsed.ok) {
    run.subcommand_state = void 0;
    return { kind: "done", body: `ewh design: could not read proposal: ${parsed.error}` };
  ...
  if (report.kind === "result") {
    ...
    return buildRefinerInstruction({
      ...
      stagedPath,
      instruction,
      existingPath: artifact.op === "update" ? existingPathForArtifact(artifact, opts) : void 0
    });
  }
  throw new Error(`design file_gate: unexpected report kind '${report.kind}'`);
}
async function continueRefine(run, sub, report, opts) {
  if (report.kind === "error") {
...
async function continueWorkflowInterview(run, sub, report, opts) {
  ...
  if (report.result_path !== sub.shape_path) {
    await fs18.mkdir(dirname6(sub.shape_path), { recursive: true });
    await fs18.copyFile(report.result_path, sub.shape_path);
  }
...
async function continueWorkflowTemplateGate(run, sub, report, opts) {
  ...
  if (report.decision === "yes") {
    ...
    await fs18.mkdir(dirname6(sub.shape_path), { recursive: true });
    await fs18.writeFile(sub.shape_path, JSON.stringify(templateDraft, null, 2), "utf8");
  }
...
async function pathExists2(path2) {
  try {
    await fs18.access(path2);
    return true;
  } catch {
    return false;
  }
}
async function startDesignModify(opts) {
...
function parseModifyTarget(spec) {
...
async function writeModifyContext(args) {
  const sections = [];
  ...
  if (args.contractPath) {
    const raw = await fs18.readFile(args.contractPath, "utf8");
  ...
  const rules = args.catalog.filter((e) => e.type === "rule").map((e) => e.name).sort();
  ...
  sections.push("");
  await fs18.writeFile(args.contextPath, sections.join("\n"), "utf8");
}
...
async function tryLoadAssetBody(candidates) {
  for (const p of candidates) {
    try {
      return await fs18.readFile(p, "utf8");
    } catch {
    }
  }
  return null;
...
async function continueModifyFerry(run, sub, report, opts) {
  if (report.kind !== "result") {
    throw new Error(
  ...
  try {
    ...
    return {
      kind: "done",
      body: `ewh design modify: proposed.json invalid: ${err instanceof Error ? err.message : String(err)}`
    };
  }
...
async function continueModifyReview(run, sub, report, opts) {
  if (report.kind !== "decision") {
    throw new Error(
  ...
  if (report.decision === "no") {
    try {
      await fs18.unlink(sub.proposed_path);
    } catch {
    }
    const ferryState = {
      kind: "design",
...
async function startManage(opts) {
  ...
  try {
    ...
    return {
      ...
      instruction: {
        kind: "done",
        body: `ewh manage: could not load contract: ${err instanceof Error ? err.message : String(err)}`
      }
    };
  }
  ...
  await fs19.mkdir(dirname7(draftPath), { recursive: true });
...
async function continueManage(run, report, opts) {
  const sub = run.subcommand_state;
...
async function applyFieldReport(field, step, report) {
  switch (field) {
    case "context":
    case "produces":
    case "requires":
...
async function applyListOrEditReport(field, step, report) {
  if (report.kind === "decision") {
  ...
  let raw;
  try {
    raw = await fs19.readFile(report.result_path, "utf8");
  } catch (err) {
    return {
      ok: false,
      errorNote: `could not read ${report.result_path}: ${err instanceof Error ? err.message : String(err)}`
    };
  }
  if (field === "script") {
  ...
  try {
    ...
    return {
      ok: false,
      errorNote: `result file is not valid JSON: ${err instanceof Error ? err.message : String(err)}`
    };
  }
  if (field === "context") {
    const v2 = validateContextArray2(parsed);
    if (!v2.ok) return { ok: false, errorNote: v2.error };
    step.context = v2.value;
    return { ok: true };
  }
  if (field === "produces") {
    const v2 = validateStringArray2(parsed, "produces");
    if (!v2.ok) return { ok: false, errorNote: v2.error };
    step.produces = v2.value;
    return { ok: true };
  }
  const v = validateRequiresArray2(parsed);
  if (!v.ok) return { ok: false, errorNote: v.error };
  step.requires = v.value;
  return { ok: true };
}
function applyGateReport(step, report) {
  if (report.kind !== "decision") {
...
function applyChunkedReport(step, report) {
  if (report.kind !== "decision") {
    return { ok: false, errorNote: `expected --decision yes|no, got ${report.kind}` };
  }
...
function applyScriptFallbackReport(step, report) {
  if (report.kind !== "decision") {
    return { ok: false, errorNote: `expected --decision yes|no, got ${report.kind}` };
  }
  if (report.decision === "no") {
...
async function renderFieldPrompt(sub, ctx, errorNote) {
  const draft = await readDraft(sub.draft_path);
  const step = draft.steps[sub.step_index];
...
async function renderContextPrompt(_sub, step, draft, header, editFile, ctx) {
  const rules = await listAvailableRules(ctx.projectRoot, ctx.pluginRoot);
  const upstreamArtifacts = draft.steps.slice(0, _sub.step_index).flatMap((s) => s.produces);
  const defaults2 = await resolveAgentDefaults(
    step.agent,
    ctx.pluginRoot,
    ctx.projectRoot
  );
  const currentRuleRefs = step.context.filter((e) => e.type === "rule").map((e) => e.ref);
  const preselected = new Set(
    currentRuleRefs.length > 0 ? currentRuleRefs : defaults2
  );
  const body = [
    ...header,
    ...
    rules.length > 0 ? rules.map((r) => `  ${preselected.has(r) ? "[x]" : "[ ]"} ${r}`).join("\n") : "  (none found under rules/ or .claude/rules/)",
    "",
    "Available upstream artifacts (type: artifact):",
    upstreamArtifacts.length > 0 ? upstreamArtifacts.map((a) => `  ${a}`).join("\n") : "  (no earlier step declares any produces[])",
    "",
    `Pre-selected rules from agent '${step.agent}'.default_rules: ${defaults2.length > 0 ? defaults2.join(", ") : "(none)"}`,
    "",
    "Choose:",
...
function renderProducesPrompt(_sub, step, header, editFile, ctx) {
  const current = step.produces.length > 0 ? step.produces.map((p) => `  - ${p}`).join("\n") : "  (none)";
  const body = [
    ...header,
    "Current produces[]:",
    current,
...
function renderGatePrompt(_sub, step, header, ctx) {
...
function renderRequiresPrompt(_sub, step, header, editFile, ctx) {
  const current = step.requires.length > 0 ? JSON.stringify(step.requires, null, 2) : "  (none)";
  const body = [
    ...header,
    "Current requires[]:",
    current,
...
function renderChunkedPrompt(_sub, step, header, ctx) {
...
function renderScriptPrompt(_sub, step, header, editFile, ctx) {
  const body = [
    ...header,
...
function renderScriptFallbackPrompt(_sub, step, header, ctx) {
...
async function readDraft(path2) {
  const raw = await fs19.readFile(path2, "utf8");
  return JSON.parse(raw);
}
async function atomicWriteJson(path2, value) {
  await atomicWriteText(path2, JSON.stringify(value, null, 2) + "\n");
}
async function atomicWriteText(path2, body) {
  await fs19.mkdir(dirname7(path2), { recursive: true });
  const tmp = `${path2}.tmp-${randomBytes4(4).toString("hex")}`;
  const fh = await fs19.open(tmp, "w");
  try {
    await fh.writeFile(body, "utf8");
    await fh.sync();
  } finally {
...
async function resolveAgentDefaults(agentName, pluginRoot, projectRoot) {
  try {
...
function formatContextList(ctx) {
  if (ctx.length === 0) return "  (none)";
  return ctx.map((e) => `  - ${e.type}: ${e.ref}`).join("\n");
}
function relativeToProject(projectRoot, path2) {
  if (path2.startsWith(projectRoot + "/")) return path2.slice(projectRoot.length + 1);
  return path2;
}
function validateContextArray2(input) {
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    out.push({ type: r.type, ref: r.ref });
  }
  return { ok: true, value: out };
}
function validateStringArray2(input, name) {
...
function validateRequiresArray2(input) {
...
async function startExpandTools(opts) {
  ...
  const state = { kind: "expand-tools", phase: "discover" };
  const intentLine = opts.description ? `User intent: ${opts.description}` : "User intent: (none supplied \u2014 ask the user if ambiguous)";
  const existingSummary = Object.keys(existing).length > 0 ? [
    "",
    "Existing expansions (new tools will be merged as a union):",
    ...Object.entries(existing).map(
      ([agent, v]) => `  ${agent} (${v.source ?? "unknown source"}): ${v.add.length} tool(s)`
    )
  ].join("\n") : "";
  return {
    state,
...
async function continueExpandTools(run, report, opts) {
  const sub = run.subcommand_state;
...
function toolsScratchPath(projectRoot) {
  return join18(projectRoot, ".ewh-artifacts", "expand-tools-available.txt");
}
...
async function readProposal(path2) {
  ...
  const obj = parsed;
  const source = typeof obj.source === "string" ? obj.source : void 0;
  ...
  const assignments = {};
  for (const [agent, tools] of Object.entries(rawAssign)) {
    if (!Array.isArray(tools)) continue;
    const names = tools.filter((t) => typeof t === "string" && t.length > 0);
...
async function readExistingAgentTools(projectRoot) {
  const state = await readEwhStateFile(projectRoot);
  return state.agent_tools ?? {};
}
async function persistAgentTools(projectRoot, proposal) {
  const state = await readEwhStateFile(projectRoot);
  ...
  for (const [agent, tools] of Object.entries(proposal.assignments)) {
    ...
    const unionTools = Array.from(/* @__PURE__ */ new Set([...existing.add ?? [], ...tools]));
    current[agent] = {
      add: unionTools,
...
async function generateAgentOverrides(projectRoot, pluginRoot, agents, merged) {
  const out = [];
...
async function readPluginAgentTools(pluginRoot, agent) {
  try {
...
function buildOverrideFile(agent, tools) {
...
function formatAge(iso, now) {
  ...
  const deltaSec = Math.max(0, Math.floor((now.getTime() - then) / 1e3));
...
function compareDrift(expectedTool, entries) {
  if (expectedTool === void 0) return "ok";
  const primary = entries.find(
    (e) => !READ_ONLY_TOOLS.has(e.tool) && (e.event === "SubagentStart" || e.event === "PostToolUse")
  );
...
// src/commands/report.ts
function extractInstructedTool(instr) {
  if (instr.kind === "bash") return "Bash";
  if (instr.kind !== "tool-call") return void 0;
...
async function runReport(opts) {
  ...
  const turnLogPath = join20(runDir(opts.projectRoot, run.run_id), "turn-log.jsonl");
  ...
  if (run.last_instructed_tool) {
    ...
    run.last_instructed_tool = void 0;
    if (driftResult !== "ok") {
  ...
  if (run.subcommand) {
    const instr2 = await dispatchSubcommandReport(run, effectiveReport, opts);
    if (instr2.kind === "done") {
      run.status = run.status === "aborted" ? "aborted" : "complete";
      await writeRunState(opts.projectRoot, run);
      await clearActive(opts.projectRoot, run.run_id);
      return formatInstruction({ ...instr2, report_with: void 0 });
    }
    run.last_instructed_tool = extractInstructedTool(instr2);
    await writeRunState(opts.projectRoot, run);
    return formatInstruction(
      instr2.report_with ? instr2 : { ...instr2, report_with: `ewh report --run ${run.run_id} --step 0` }
    );
  }
  if (run.manage_scripts_pending) {
    ...
    run.manage_scripts_pending = void 0;
    ...
    let instr2 = r.instruction;
    if (instr2.report_with === "__CONTINUE__" || instr2.kind === "done") {
      instr2 = await driveForward(run, instr2, transOpts);
    }
    if (instr2.kind === "done") {
      run.status = "complete";
      await writeRunState(opts.projectRoot, run);
    ...
    await writeRunState(opts.projectRoot, run);
    return formatInstruction(
      instr2.report_with ? instr2 : { ...instr2, report_with: `ewh report --run ${run.run_id} --step ${run.current_step_index}` }
    );
  }
  ...
  if (instr.kind === "done") {
    ...
    await writeRunState(opts.projectRoot, run);
    await clearActive(opts.projectRoot, run.run_id);
    return formatInstruction({ ...instr, report_with: void 0 });
  }
  run.last_instructed_tool = extractInstructedTool(instr);
  await writeRunState(opts.projectRoot, run);
  return formatInstruction(
    instr.report_with ? instr : {
      ...instr,
      report_with: `ewh report --run ${run.run_id} --step ${run.current_step_index}`
    }
  );
}
async function dispatchSubcommandReport(run, report, opts) {
...
async function driveForward(run, last, opts) {
  let current = last;
  for (let i = 0; i < 1e3; i++) {
...
async function main(argv) {
  ...
  if (values.abort) {
    ...
    report = {
      ...
      result_path: typeof values.result === "string" ? values.result : void 0
    };
  }
  const out = await runReport({ projectRoot, pluginRoot, runId, stepIndex, report });
  process.stdout.write(out);
}
function requireStr(v, flag) {
  if (typeof v !== "string" || v.length === 0) {
    throw new Error(`missing required flag ${flag}`);
  }
  return v;
}
function parseStepIndex(v) {
  ...
  for (const name of entries) {
    ...
    try {
      ...
      for (const step of wf.steps) {
        if (step.agent) {
          const found = await fileExists(join21(projectRoot, ".claude", "agents", `${step.agent}.md`)) || await fileExists(join21(pluginRoot, "agents", `${step.agent}.md`));
          if (!found) {
            issues.push(
        ...
        for (const ruleName of step.rules ?? []) {
          const pluginMatches = await findRuleFiles2(ruleName, join21(pluginRoot, "rules"));
...
async function main3(argv) {
  ...
  const projectRoot = values["project-root"] ?? process.env.CLAUDE_PROJECT_ROOT ?? process.cwd();
  ...
  const runDir2 = await findActiveRunDir(projectRoot);
  if (!runDir2) {
    return;
  }
  ...
  if (eventName === "SubagentStart") {
    ...
    record = {
      ...
      ts,
      tool: "Agent",
      ...toolInput?.subagent_type !== void 0 ? { subagent_type: toolInput.subagent_type } : {},
      ...toolInput?.description !== void 0 ? { description: toolInput.description } : {}
    };
  } else if (eventName === "SubagentStop") {
    record = { event: "SubagentStop", ts, tool: "Agent" };
  } else if (eventName === "PostToolUse") {
    const toolName = stdinJson.tool_name ?? "unknown";
